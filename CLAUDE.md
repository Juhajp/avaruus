...
- **Maan tiet ja kaupungit**: tieverkko ROAD_SP=900 yks ruudukossa (`roadDist`), osa risteyksistä kaupunkeja (`townAt`). Maasto tasoittuu teiden/kaupunkien kohdalla (surfHeightFn), laatat värjäävät tiet asfaltiksi, rakennukset ovat instansoitua sirotetta julkisivutekstuurilla (`getBuildingTextures`) — ikkunat syttyvät yöllä (`bldgMat.emissiveIntensity` updateDaylightissa). Puut/kivet väistävät teitä ja kaupunkeja (`inTownArea`)
